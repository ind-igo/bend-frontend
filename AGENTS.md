...
- Keep the Bend frontend target-independent. Backends stay in `backends/` as explicit consumers. The JavaScript host may use upstream main.ts/comp.ts to bootstrap our Bend tools, but must never compile or execute the user input during checking.
